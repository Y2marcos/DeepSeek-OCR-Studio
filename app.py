...
# UI: detect device for display (does not change model/device behavior)
...
with gr.Blocks(theme=gr.themes.Soft(), title="DeepSeek-OCR") as demo:
    # small CSS to keep the demo centered and a nice max width
    gr.HTML("""
    <style>
      .gradio-container { max-width: 1100px; margin: 0 auto; }
      .ds-note { font-size:14px; color:#555 }
    </style>
    """)
    ...
    with gr.Row():
        with gr.Column(scale=3):
            ...
            **Convert documents to markdown, extract raw text, and locate specific content with bounding boxes.**
            ...
            It typically takes ~20s for markdown and ~3s for the locate task. Use the controls on the left to upload files and choose a mode/task.
            """)

        with gr.Column(scale=1):
            # compact model/device info box (visual only)
            gr.Markdown(f"""
            **Model:** `{MODEL_NAME}`  
            **Device:** `{device_str}`  
            """)
    ...
    with gr.Row():
        with gr.Column(scale=1):
            ...
            gr.Markdown("<div class='ds-note'>Mode controls base tile size and whether the model will use cropping; choose a larger mode for higher quality.</div>", elem_id=None)
            task = gr.Dropdown(list(TASK_PROMPTS.keys()), value="📋 Markdown", label="Task")
            gr.Markdown("<div class='ds-note'>Task selects the output behavior: Markdown converts documents, Locate finds specific text, etc.</div>", elem_id=None)
            prompt = gr.Textbox(label="Prompt", lines=2, visible=False, placeholder="Add <|grounding|> for boxes when needed")
    ...
    gr.Examples(
        examples=[
            ...
            ["examples/Screenshot 2023-11-27 032658.png", "Gundam", "📍 Locate", "Robot"]
        ],
